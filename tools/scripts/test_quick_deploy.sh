...
echo "# Test comment added at $(date)" >> platform/runtime/cli/src/main.py
...
backup_cmd = "ssh rocksteady 'cd ~/unibos/platform/runtime/web/backend && cp -f unibos_platform/runtime/web/backend/settings/production.py /tmp/prod_bak.py 2>/dev/null; cp -f .env /tmp/env_bak 2>/dev/null'"
...
if result.returncode == 0:
    ...
    restore_cmd = "ssh rocksteady 'cd ~/unibos/platform/runtime/web/backend && [ -f /tmp/prod_bak.py ] && cp -f /tmp/prod_bak.py unibos_platform/runtime/web/backend/settings/production.py; [ -f /tmp/env_bak ] && cp -f /tmp/env_bak .env'"
...
# Remove test comment
sed -i '' '/# Test comment added at/d' platform/runtime/cli/src/main.py 2>/dev/null || sed -i '/# Test comment added at/d' platform/runtime/cli/src/main.py
