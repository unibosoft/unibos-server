...
echo ""
echo 'rsync -avz platform/runtime/web/backend/ rocksteady:~/unibos/platform/runtime/web/backend/ && ssh rocksteady "cd ~/unibos/platform/runtime/web/backend && source venv/bin/activate 2>/dev/null || python3 -m venv venv && source venv/bin/activate && pip install -q -r requirements.txt && python manage.py migrate --noinput && python manage.py runserver 0.0.0.0:8000"'
echo ""
echo "─────────────────────────────────────────────────────────────"
echo ""
echo "5️⃣  Deploy only CLI:"
echo ""
echo 'rsync -avz platform/runtime/cli/src/ rocksteady:~/unibos/platform/runtime/cli/src/ && ssh rocksteady "cd ~/unibos/platform/runtime/cli/src && python3 main.py"'
echo ""
echo "─────────────────────────────────────────────────────────────"
echo ""
...
echo ""
echo 'rsync -avz . rocksteady:~/unibos/ && ssh rocksteady "cd ~/unibos && sudo cp platform/runtime/web/backend/unibos.service /etc/systemd/system/ && sudo systemctl daemon-reload && sudo systemctl restart unibos && sudo systemctl status unibos"'
echo ""
echo "─────────────────────────────────────────────────────────────"
echo ""
