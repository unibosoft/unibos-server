...
if pgrep -f "manage.py runserver" > /dev/null; then
    ...
    echo "Logs at: ~/unibos/platform/runtime/web/backend/logs/server.log"
else
    echo "❌ Server failed to start"
    echo "Check logs: ssh rocksteady 'tail -n 50 ~/unibos/platform/runtime/web/backend/logs/server.log'"
fi
EOF
